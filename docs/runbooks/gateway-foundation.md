...
В объекте `larnitech` нормальное соединение имеет `status: online`, адреса
`315:36` и `456:249` в `subscribed_addrs` и актуальный `heartbeat_at`. Поле
`last_event_at` меняется только при фактическом событии подписанного устройства;
отсутствие изменений само по себе не означает потерю соединения.
