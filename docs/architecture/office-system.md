...
`456:249`. Бинарное событие AC уточняется запросом подробного статуса.
CO2-автоматика реализована сценарием Larnitech `315:245`: при
`CO₂ >= 1200 ppm` она выбирает летнюю рекуперацию VAKIO на скорости 1, а при
`CO₂ <= 1000 ppm` выключает питание. Ручное управление выполняется через
область `VAKIO` в Larnitech. Прямой HTTP endpoint шлюза при активном мосте
запрещён.
