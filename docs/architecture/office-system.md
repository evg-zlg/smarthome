...
передаются состояния и команды и где остаётся ручное управление. Схема сочетает
фактически проверенную основу и целевую интеграцию вентиляции. Проектные связи
не следует считать действующими до отдельной проверки на объекте.
...
flowchart LR
    ...
    User -->|HTTP health без управления| Health
...
`*` Точная локальная шина устройств Larnitech, модель VAKIO и её команды пока
не подтверждены. Эти участки показаны концептуально.
...
| Raspberry Pi 3B+ | Локальный интеграционный шлюз | События Larnitech, телеметрия VAKIO | Read-only карта и health status | Работает; управление выключено |
| Larnitech adapter | Преобразование API2 в внутреннюю модель | События API2 WebSocket | Нормализованные показания с временем обновления | Работает в read-only режиме |
...
| Mosquitto | Локальная доставка сообщений | Состояния и команды только авторизованных клиентов | MQTT-сообщения по ACL | Работает только на `localhost`; LAN-доступ выключен |
| VAKIO adapter | Перевод внутренней команды в протокол конкретной модели | Желаемый режим и обратное состояние | Команды VAKIO и подтверждение результата | Запланирован |
| VAKIO | Исполнитель вентиляции | Штатное ручное управление; в будущем локальные MQTT-команды | Фактический режим и состояние | Модель и интерфейс не подтверждены |
...
постоянное WebSocket-соединение и подписку на CO2 `315:36`. Логика управления
вентиляцией ещё не включена, `control_enabled=false`.
...
2. Точную модель, прошивку, локальный интерфейс и безопасное состояние VAKIO.
3. Совместимость локального MQTT со штатным ручным управлением VAKIO.
