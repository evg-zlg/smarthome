...
- Nginx слушает TCP `80` на IPv4 и IPv6; опубликованы локальная панель,
  `/healthz`, `/gateway/healthz`, `/gateway/map` и POST-маршрут
...
- `GET http://127.0.0.1/gateway/healthz` возвращает актуальную карту Larnitech,
  состояние MQTT/VAKIO и `control_enabled=true`.
...
- состояние, режим и скорость VAKIO, а также ручные кнопки включения и
  выключения с подтверждением команды по MQTT;
...
- Автоматическая логика по CO2 ещё не включена: события используются только
  для наблюдения, ручное управление разрешено исключительно для VAKIO.
