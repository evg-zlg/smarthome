...
- Реестр панели содержит ровно семь исследованных Larnitech-сценариев. На дату
  проверки `LARNITECH_SCENARIO_CONTROL_ENABLED=false`, runtime-allowlist пуст,
  поэтому все кнопки заблокированы, а POST возвращает `403` до API2.
...
- Управление VAKIO включено отдельно; управление сценариями Larnitech остаётся
  отключённым до выбора runtime-allowlist и отдельного физического теста.
