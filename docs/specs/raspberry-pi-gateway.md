...
- LAN-listener ограничен ACL `vakio/#` и nftables до источника
  `192.168.1.164`; остальные подключения к LAN-адресу TCP/1883 отклоняются.
- Gateway подключается к `127.0.0.1` под отдельным пользователем.

Целевая схема развёрнута: аутентифицированный
listener `127.0.0.1:1883` для gateway и анонимный `192.168.1.183:1883`
для штатного MQTT-клиента VAKIO. Второй listener ограничивается
nftables до IP `.164` и ACL до `vakio/#`.
...
- Автоматика CO2 отключена до согласования порогов и временных
  ограничений.
- Физическое помещение датчиков Larnitech ещё не подтверждено.
